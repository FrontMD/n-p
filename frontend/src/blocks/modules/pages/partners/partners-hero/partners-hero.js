document.addEventListener('DOMContentLoaded', function() {
  const downloadItems = document.querySelectorAll('.partenrs__list-item');
  if (downloadItems.length === 0) return;

  // Функция для обновления высоты элемента
  function updateItemHeight(item) {
    const itemTop = item.querySelector('.partenrs__list-item__top');
    const itemBottom = item.querySelector('.partenrs__list-item__bottom');
